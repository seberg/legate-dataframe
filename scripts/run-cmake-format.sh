...
status=0
if [ -z ${LEGATE_DATAFRAME_ROOT:+PLACEHOLDER} ]; then
    LEGATE_DATAFRAME_BUILD_DIR=$(git rev-parse --show-toplevel 2>&1)/cpp/build
    status=$?
else
    LEGATE_DATAFRAME_BUILD_DIR=${LEGATE_DATAFRAME_ROOT}
fi

if ! [ ${status} -eq 0 ]; then
    if [[ ${LEGATE_DATAFRAME_BUILD_DIR} == *"not a git repository"* ]]; then
        echo "This script must be run inside the repository, or the LEGATE_DATAFRAME_ROOT environment variable must be set."
    else
        echo "Script failed with unknown error attempting to determine project root:"
        echo ${LEGATE_DATAFRAME_BUILD_DIR}
    fi
    exit 1
fi

DEFAULT_FORMAT_FILE_LOCATIONS=(
  "${LEGATE_DATAFRAME_BUILD_DIR:-${HOME}}/_deps/rapids-cmake-src/cmake-format-rapids-cmake.json"
)
